...
static double *imgcpy(const double *src, s32 width, s32 height) {
    double *dst;
    size_t size = ((size_t)width * (size_t)height * sizeof(*dst)) << 2;
    dst = malloc(size);
    if (!dst)
        return NULL;
    memcpy(dst, src, size);
...
double *sample(const double *src, s32 src_width, s32 src_height, s32 dst_width, s32 dst_height) {
    ...
    double *dst = malloc(((size_t)dst_width * (size_t)dst_height * sizeof(*dst)) << 2);
    if (!dst)
        return NULL;
...
static double *v_filter(const double *src, s32 width, s32 dst_height, const s32 *bounds, const double *coeffs, size_t support) {
    ...
    double *dst = calloc((size_t)adj_width * (size_t)dst_height, sizeof(*dst));
    if (!dst)
        return NULL;
...
static void h_iconvolve_ip(double *img, s32 width, s32 height, const double *L, int m, double c) {
    int n = m - 1;
    if (m >= width)
        n = m = width - 1;
...
static void v_iconvolve_ip(double *img, s32 width, s32 height, const double *L, int m, double c) {
    int n = m - 1;
    if (m >= height)
        n = m = height - 1;
...
static bool gen_area_filter(s32 **_bounds, double **_coeffs, size_t *_support, s32 src, s32 dst) {
    ...
    s32 *bounds = malloc(2 * (size_t)dst * sizeof(*bounds));
    if (!bounds)
        return true;

    double *coeffs = malloc(support * (size_t)dst * sizeof(*coeffs));
    if (!coeffs) {
        free(bounds);
        return true;
    }
